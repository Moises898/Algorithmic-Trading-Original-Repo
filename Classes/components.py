...
def login_for_license_screen(frame):
    ...
    frame.main_frame.message = customtkinter.CTkLabel(frame.main_frame, text="Welcome", font=customtkinter.CTkFont(size=30, weight="bold"))
    frame.main_frame.message.grid(row=0, column=0, padx=(20,20), pady=(30,5))
    frame.main_frame.message_2 = customtkinter.CTkLabel(frame.main_frame, text="Please authenticate before start", font=customtkinter.CTkFont(size=16, weight="normal"))
    ...
    image_label.grid(row=5,column=0, sticky="nsew",padx=(15, 15), pady=(30, 60))
...
def connection_mt5_screen(frame):
    ...
    frame.main_frame.connect.grid(row=5, column=0,pady=10,rowspan=2)    
    ...
    frame.main_frame.message_3.grid(row=7, column=0, padx=(20,20), pady=(0,20),sticky="ew")
...
def strategy_running_screen(frame):
    ...
    def update_profit(object):    
        if not object.stop_thread_flag.is_set():                       
            ...
            object.main_frame.profit_or_loss.grid(row=3, column=0,columnspan=2, padx=PADX, pady=2, sticky="ew")    
            if not object.stop_thread_flag.is_set():                       
...
def start_strategy_mt5_screen(frame):
    ...
    lots_value = round(lots_value / 1.5, 2)
    ...
    if len(symbols) != 0:       
        ...
        image_label.grid(row=2, column=0, sticky="nsew", padx=0, pady=30)