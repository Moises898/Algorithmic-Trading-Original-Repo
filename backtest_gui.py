...
def generate_tables_of_trades(chart, results):
    ...
    for idx, key in enumerate(trades.keys()):
        ...
        if result == "WIN":
            ...
            win_operations += 1
            table.new_row(idx, result, round(trades[key]["df"]["TP"].iloc[0], 2 if symbol == "XAUUSD" else 4),
                          round(trades[key]["df"]["SL"].iloc[0], 2 if symbol == "XAUUSD" else 4), trades[key]["points"])
        else:
            loss_points += trades[key]["sl_points"]
            loss_operations += 1
            table.new_row(idx, result, round(trades[key]["df"]["TP"].iloc[0], 2 if symbol == "XAUUSD" else 4),
                          round(trades[key]["df"]["SL"].iloc[0], 2 if symbol == "XAUUSD" else 4),
                          trades[key]["sl_points"])
